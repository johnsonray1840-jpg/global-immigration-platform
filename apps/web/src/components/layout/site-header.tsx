...
export default function SiteHeader() {
  ...
  const mainNav = [
    ...
    { label: t('nav.countries'), href: '/countries', icon: Globe2 },
    { label: 'Compare', href: '/compare', icon: FileText },
    { label: t('nav.eligibility'), href: '/eligibility', icon: ShieldCheck },
    { label: t('nav.packages'), href: '/packages', icon: Package },
    { label: t('nav.scholarships'), href: '/scholarships', icon: GraduationCap },
  ...
  const programLinks = [
    ...
    { label: t('purpose.permanent-residence'), href: '/programs/permanent-residence' },
    { label: t('purpose.citizenship') + ' by Investment', href: '/programs/citizenship-by-investment' },
    { label: t('purpose.work') + ' Permit', href: '/programs/work-permit' },
    { label: t('purpose.study') + ' Immigration', href: '/programs/student-immigration' },
    { label: t('purpose.family'), href: '/programs/family-sponsorship' },
    ...
    <header className={cn(
      ...
      <div className="container-premium flex h-16 md:h-20 items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center gap-6">
          ...
          <div className="relative" onMouseEnter={() => setProgramsOpen(true)} onMouseLeave={() => setProgramsOpen(false)}>
            <button className="flex items-center gap-1 text-sm font-medium text-gray-700 hover:text-[#0B5D66]">
              {t('nav.programs')} <ChevronDown className="h-4 w-4" />
            </button>
            {programsOpen && (
        ...
        <div className="hidden lg:flex items-center gap-4">
          ...
          {isLoggedIn ? (
            <>
              <Link href="/dashboard" className="text-sm font-medium text-gray-700 hover:text-[#0B5D66]">{t('nav.dashboard')}</Link>
              <Button variant="ghost" onClick={handleLogout}><LogOut className="mr-2 h-4 w-4" /> {t('nav.logout')}</Button>
            </>
          ) : (
            <>
              <Link href="/login" className="text-sm font-medium text-gray-700 hover:text-[#0B5D66]">{t('nav.signin')}</Link>
              <Link href="/consultation"><Button className="bg-[#0B5D66] text-white hover:bg-[#0A4E56]">{t('cta.button')}</Button></Link>
            </>
          )}
        </div>
            ...
            <SheetContent side="left" className="w-80 p-0 bg-white">
              ...
              <nav className="p-5 space-y-1 overflow-y-auto max-h-[80vh]">
                ...
                <div className="pt-3 border-t border-gray-100">
                  <p className="text-sm font-semibold text-gray-500 uppercase mb-2">{t('nav.programs')}</p>
                  {programLinks.map((link) => (
                  ...
                  {isLoggedIn ? (
                    <>
                      <Link href="/dashboard" className="flex items-center gap-3 py-2.5 text-base text-gray-800 hover:text-[#0B5D66]">
                        <LayoutDashboard className="h-5 w-5 text-[#0B5D66]" /> {t('nav.dashboard')}
                      </Link>
                      <Button variant="ghost" onClick={handleLogout} className="w-full justify-start"><LogOut className="mr-2 h-4 w-4" /> {t('nav.logout')}</Button>
                    </>
                  ) : (
                    <Link href="/consultation"><Button className="w-full bg-[#0B5D66] text-white">{t('cta.button')}</Button></Link>
                  )}
                </div>
              </nav>
            </SheetContent>
